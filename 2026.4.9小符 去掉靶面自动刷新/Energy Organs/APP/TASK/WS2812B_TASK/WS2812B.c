		...
		void my_ws2812_1_set(uint16_t leaf ,uint16_t num,uint8_t rv,uint8_t gv,uint8_t bv)
{
	...
	switch(leaf)
	{
		...
		case energe_leaf5:
			...
			{
					//填充数组
					LED_BYTE_Buffer2[indexx+i]      = (gv << i) & (0x80)?TIMING_ONE:TIMING_ZERO;
					LED_BYTE_Buffer2[indexx+i + 8]  = (rv << i) & (0x80)?TIMING_ONE:TIMING_ZERO;
					LED_BYTE_Buffer2[indexx+i + 16] = (bv << i) & (0x80)?TIMING_ONE:TIMING_ZERO;
			}
			break;
	}
}
