...
/* ----------------------- Function Implements ---------------------------- */
	...
	*/
void RemoteDataProcess(uint16_t *pData)
{
	 if(pData == NULL)
	 {
		return;
	...
	RC_CtrlData.rc.ch0   = (int16_t)(pData[4]<<8|pData[5]);
	

	RC_CtrlData.rc.ch1   = (int16_t)(pData[6]<<8|pData[7]);
	
	RC_CtrlData.rc.ch2   = (int16_t)(pData[8]<<8|pData[9]);//
	
	RC_CtrlData.rc.ch3	 = (int16_t)(pData[10]<<8|pData[11]);
	 //your control code ….
}
