...
//	if(RC_CtrlData.rc.ch0  <1800)
//			GPIO_ResetBits(GPIOC,GPIO_Pin_10);
//		if(RC_CtrlData.rc.ch0  >2000)
...
//	tData[5]=RC_CtrlData.rc.ch0>>8|RC_CtrlData.rc.ch1;
    ...
    tData[5] |= (uint8_t)((RC_CtrlData.rc.s2 & 0x03) << 4); 
	 RC_mcu1data.rc.ch0 = ((int16_t)tData[0] | ((int16_t)tData[1] << 8)) & 0x07FF; 
	 RC_mcu1data.rc.ch1 = (((int16_t)tData[1] >> 3) | ((int16_t)tData[2] << 5)) & 0x07FF;
	 RC_mcu1data.rc.ch2 = (((int16_t)tData[2] >> 6) | ((int16_t)tData[3] << 2) |((int16_t)tData[4] << 10)) & 0x07FF;
	 RC_mcu1data.rc.ch3 = (((int16_t)tData[4] >> 1) | ((int16_t)tData[5]<<7)) & 0x07FF;	 
	 RC_mcu1data.rc.s1 = ((tData[5] >> 4) & 0x000C) >> 2;
	 RC_mcu1data.rc.s2 = ((tData[5] >> 4) & 0x0003);
	 //接受就给串口4发送数据
	 uint16_t T_Buf[18]={0xee,0xff,pData[0],pData[1],pData[2],pData[3],pData[4],
		           pData[5],pData[6],pData[7],pData[8],pData[9],pData[10],pData[11],0,0,0,0xfe};
	//	USART4_SendData18(T_Buf);
} 


void USART3_IRQHandler()
{
	static uint32_t USART3_DMA_current_rx_len = 0;
	if(USART_GetITStatus(USART3, USART_IT_IDLE) != RESET)
    {
		(void)USART3->SR;
		(void)USART3->DR;
		if(DMA_GetCurrentMemoryTarget(DMA1_Stream1) == 0/*当前目标为存储器0时*/)
        {
            DMA_Cmd(DMA1_Stream1, DISABLE);
            DMA_ClearFlag(DMA1_Stream1, DMA_FLAG_TCIF2 | DMA_FLAG_HTIF2);
            USART3_DMA_current_rx_len = USART3_DMA_RX_BUF_LEN - DMA_GetCurrDataCounter(DMA1_Stream1);//接收数据的长度
            // 将内存指针重新定位到起始位置
            DMA1_Stream1->NDTR = (uint16_t)USART3_DMA_RX_BUF_LEN; //NDTR寄存器表示DMA还没有转移的数据量
            // 启用当前选择的内存为memory 1
            DMA1_Stream1->CR |= (uint32_t)(DMA_SxCR_CT); 
            DMA_Cmd(DMA1_Stream1, ENABLE);
            // 确保接收到完整的数据帧
            if(USART3_DMA_current_rx_len == ST_LENGTH) //如果前面的数据转移完
            {

              RemoteStData(USART3_DMA_RX_BUF[0],USART3_DMA_TX_BUF[0]);//数据转移
        ...
        else/*当前目标为存储器1时*/
        {
            DMA_Cmd(DMA1_Stream1, DISABLE);
            DMA_ClearFlag(DMA1_Stream1, DMA_FLAG_TCIF2 | DMA_FLAG_HTIF2);
            USART3_DMA_current_rx_len = USART3_DMA_RX_BUF_LEN - DMA_GetCurrDataCounter(DMA1_Stream1);
            DMA1_Stream1->NDTR = (uint16_t)USART3_DMA_RX_BUF_LEN; // 将内存指针重新定位到起始位置
            DMA1_Stream1->CR &= ~(uint32_t)(DMA_SxCR_CT);   // 启用当前选择的内存为memory 1
            DMA_Cmd(DMA1_Stream1, ENABLE);
            if(USART3_DMA_current_rx_len == ST_LENGTH)
            {

                RemoteStData(USART3_DMA_RX_BUF[1],USART3_DMA_TX_BUF[1]);//接收数据
