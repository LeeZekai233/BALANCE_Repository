...
uint8_t _UART5_DMA_RX_BUF[100];
	...
	static uint8_t _USART2_DMA_RX_BUF[BSP_USART2_DMA_RX_BUF_LEN];//双缓冲接收区
...
void usart2_init(uint32_t baud)
{
    ...
    DMA_InitStructure.DMA_BufferSize = sizeof(USART2_DMA_TX_BUF);
    ...
    DMA_Cmd(DMA1_Stream6, ENABLE);  // 这里必须使能
    
    /* 接收DMA */
    USART_DMACmd(USART2, USART_DMAReq_Rx, ENABLE);
    
    DMA_StructInit(&DMA_InitStructure);
	DMA_InitStructure.DMA_BufferSize = sizeof(_USART2_DMA_RX_BUF);
	DMA_InitStructure.DMA_Channel = DMA_Channel_4;
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
	DMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Disable;
	DMA_InitStructure.DMA_FIFOThreshold = DMA_FIFOThreshold_1QuarterFull;
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)_USART2_DMA_RX_BUF;
	DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_Single;
	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
	DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;
