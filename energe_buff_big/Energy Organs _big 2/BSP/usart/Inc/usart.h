...
#define EN_USART1 									    0//默认双缓冲

#define EN_USART2       						  	1//默认双缓冲

#define EN_USART3												0//默认单缓冲

#define EN_UART4												0//默认单缓冲

#define EN_UART5												1//可选缓冲
#define EN_UART5_DMA_SECOND_FIFO 				1

#define EN_USART6												0//默认单缓冲
...
*********************************************************************************************************
*/
#define USART1_Data_Receive_Process_0				do{RemoteDataProcess(_USART1_DMA_RX_BUF[0]);}while(0);																																															
#define USART1_Data_Receive_Process_1				do{RemoteDataProcess(_USART1_DMA_RX_BUF[1]);}while(0);

#define USART2_Data_Receive_Process_0				do{RemoteDataProcess(_USART2_DMA_RX_BUF[0]);}while(0);
#define USART2_Data_Receive_Process_1				do{RemoteDataProcess(_USART2_DMA_RX_BUF[1]);}while(0);
...
#define BSP_USART6_RX_BUF_LENGTH  100
	...
	static uint16_t _USART2_DMA_RX_BUF[2][BSP_USART2_DMA_RX_BUF_LEN];//双缓冲接收区
#define UART4_TX_BUF_LENGTH       100
#define UART5_TX_BUF_LENGTH       150

#define RC_FRAME_LENGTH           8
