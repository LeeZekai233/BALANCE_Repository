...
/********************** 变量定义*********************/
extern int game_stage;  // 引用 control_task 中的变量，用于进度显示

int current_r = 150;  // 150
int side_r = 254;     // 254
...
void transfer1(void) {
  ...
  // until transfer complete TIM_CCxCmd(TIM5, TIM_Channel_1, TIM_CCx_Disable);
...
void transfer4(void) {
  ...
  DMA_ClearFlag(DMA1_Stream1,
                DMA_FLAG_TCIF1);  // clear DMA1 Channel 6 transfer complete flag
}
void transfer5(void) {
  DMA_SetCurrDataCounter(
      DMA1_Stream5,
  ...
  DMA_ClearFlag(DMA1_Stream5,
                DMA_FLAG_TCIF5);  // clear DMA1 Channel 6 transfer complete flag
}
// dma尚未配置
...
void my_ws2812_1_set(uint16_t leaf,
  ...
  switch (leaf) {
    ...
    case energe_leaf5:
      for (uint8_t i = 0; i < 8; i++) {
        // 填充数组，注意是buf2
        LED_BYTE_Buffer2[indexx + i] =
            (gv << i) & (0x80) ? TIMING_ONE : TIMING_ZERO;
        LED_BYTE_Buffer2[indexx + i + 8] =
            (rv << i) & (0x80) ? TIMING_ONE : TIMING_ZERO;
        LED_BYTE_Buffer2[indexx + i + 16] =
            (bv << i) & (0x80) ? TIMING_ONE : TIMING_ZERO;
      }
      break;
  }
...
void LED_circle(uint16_t leaf) {
  for (int i = CIRCLE6_START + 1; i <= MY_WS2812_MAX_NUM; i++) {
    my_ws2812_1_set(leaf, i, 222, 0, 0);
  }
}
    ...
    case Big_energe:
      switch (LED_mode[leaf])  // 小能量机关
      {
        ...
        case Waiting_hit:
          ...
          side_off(leaf);     // 侧面关
          LED_circle(leaf);   // 靶子图案 (此处已改为进度条逻辑)
          break;
        case hit_finish:
    ...
    case Small_energe:
      switch (LED_mode[leaf])  // 大能量机关
      {
        ...
        case Waiting_hit:
          LED_current(leaf);  // 流水箭头
          side_off(leaf);     // 侧面关
          LED_circle(leaf);   // 靶子x图案 (此处已改为进度条逻辑)
          break;
        case hit_finish:
          LED_current_all_on(leaf);  // 箭头流水全亮
          side_off(leaf);            // 侧面亮
