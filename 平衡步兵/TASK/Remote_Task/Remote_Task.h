...
{
	int Cnt;//计数，用来判断长短按
	uint8_t  Original_Short_Press_Flag;//原始短按标志位,只用来判断按键状态
    uint8_t  Short_Press_Flag;//短按标志位
    uint8_t  Long_Press_Flag;//长按标志位
    uint8_t  Last_Original_Short_Press_Flag;//上一次的原始短按标志位，来检测上升下降沿去判断有没有重新按下，用来判断是否反转按键状态
...
typedef __packed struct
{
    ...
    Key_Mouse_Action_t Trigger_Up_Action;
    Key_Mouse_Action_t Trigger_Down_Action;
...
void Key_Mouse_Action_Judge(Key_Mouse_Action_t* Key_Mouse_Action);
