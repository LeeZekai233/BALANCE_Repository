...
void Buffer_Power(void)
{
	if(can_capacitance_message.cap_voltage_filte<22.5)
		Max_Power = Super_Cap_Send.chassis_power_limit+(Super_Cap_Send.chassis_power_buffer-10); //5
	else
		Max_Power=0;
...
**/
float I6020[4];
float dpower[4],hpower[4],all_power1[4],all_power2[4],all_power;
float kall6020[4];
//float k6020,k2_6020,k1_6020,k0_6020,kall6020[4];
float Max_Power_6020,max_chassis_power,Max_Power_3508;

float get_6020power(void)//duan
{
    for(int i=0;i<4;i++)
    {
        I6020[i] = pid_calc(&chassis.pid_6020_motor_speed[i], chassis.Helm_speed_fdb[i], chassis.Helm_speed_ref[i]);//Helm_chassis.Heading_Encoder[i].Torque;
        VAL_LIMIT(I6020[i],-10000,10000);    // 6020电压最大发送值为25000
        dpower[i] = I6020[i]*chassis.pid_6020_motor_speed[i].get*k6020;
...
//	if(Chassis.Max_Power>0)
//	{
//		VAL_LIMIT(Max_Power_6020,0,Chassis.Max_Power);
//	}
//	else
//		Max_Power_6020=0;

    float static a[4],b[4],c[4];
    if(all_power>chassis.Max_Power)
    {
        for(int i = 0; i < 4; i++)
        {
            all_power2[i]=all_power1[i]*chassis.Max_Power/all_power;
            if(all_power1[i]>k0_6020&&all_power1[i]>all_power2[i]&&(b[i]*b[i]-4*a[i]*c[i])>0)//或许all_power2>k0_6020更好
            {
                a[i]=k2_6020;
                b[i]=k1_6020+k6020*chassis.pid_6020_motor_speed[i].get;
...
void Helm_Power_Limit_Handle(void)
{		
	/*↓补给区禁止用超电 防止超功率↓*/
...
//			
			...
			{
				chassis.Max_Power = Super_Cap_Send.chassis_power_limit;
			}
			if(chassis.Chassis_Move_State==CHAS_UP_SLOPE)
				chassis.Max_Power = Super_Cap_Send.chassis_power_limit+80;
			...
			if(chassis.chassis_speed_mode==HIGH_SPEED_MODE)
			{
				...
				{
					chassis.Max_Power = Super_Cap_Send.chassis_power_limit+200;
				}
				else if(chassis.Chassis_Move_State==CHAS_STATE_RUN)
				{
					chassis.Max_Power = Super_Cap_Send.chassis_power_limit+120;
				}
				else
				{
					chassis.Max_Power = Super_Cap_Send.chassis_power_limit+120;
			...
			else if(chassis.chassis_speed_mode==NORMAL_SPEED_MODE)
			{
				...
				{
					chassis.Max_Power = Super_Cap_Send.chassis_power_limit+50;
				}
			}
//				if(chassis.Chassis_Move_State == CHAS_STATE_RUN_READY)
//				chassis.Max_Power=Super_Cap_Send.chassis_power_limit+100;
//				else if(chassis.Chassis_Move_State == CHAS_STATE_RUN)
//				chassis.Max_Power=Super_Cap_Send.chassis_power_limit+50;
...
//	chassis.Max_Power=100;//超电寄时或测试用
