...
void CAN1_Receive_Task(CanRxMsg* RxMsg,Balance_Chassis_t* Chassis)
{
    ...
    {
        case 0xA1:
            DaMiao_8009_Information_Receive(RxMsg,&Joint_Motor[0],+2.5770793- 0.33405f);//+2.68839049f - 0.33405f);//-0.71786f - 0.33405f);
            DaMiao_8009_To_Generic_Encoder(&Joint_Motor[0],&Chassis->Joint_Motor[0]);//右腿phi1
            break;
        case 0xA2:
            DaMiao_8009_Information_Receive(RxMsg,&Joint_Motor[1],+0.268019199+ 0.33405f);//+0.348554611 + 0.33405f);//+2.10815334 + 0.33405f);//+ 0.33405f
            DaMiao_8009_To_Generic_Encoder(&Joint_Motor[1],&Chassis->Joint_Motor[1]);//左腿phi1
            break;
        case 0xA3:
            DaMiao_8009_Information_Receive(RxMsg,&Joint_Motor[2],-0.647397757- 0.75432f);//-0.666285276f - 0.75432f);//-2.50632334f - 0.75432f);
            DaMiao_8009_To_Generic_Encoder(&Joint_Motor[2],&Chassis->Joint_Motor[2]);//左腿phi4
            break;
        case 0xA4:
            DaMiao_8009_Information_Receive(RxMsg,&Joint_Motor[3],+1.7635777 + 0.75432f);// 1.78639603f + 0.75432f);//2.54055f + 0.75432f);
